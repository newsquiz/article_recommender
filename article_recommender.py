...
class ArticleRecommender:
    def __init__(self, k=2, mongo_uri=None, mongo_db=None, sim_func=cosine_similarity, uuCF=0):
    ...
    def get_articles_clicked_by_users(self):
        user_clicks = self.db.user_click.find({}, {'u_id': 1,
                                                   'a_id': 1})
        Y_data = []
    ...
    def get_articles_for_random(self, u, num):
        user_clicks = self.db.user_click.find(
            {'u_id': u}, {'a_id': 1, 'topic': 1})
...
def main():
    ...
    # recommender.fit()
    recommended_articles = recommender.recommend('84694', num=20)
